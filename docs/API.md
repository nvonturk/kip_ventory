...
This document details how to interact with the REST API via HTTP methods.  All API endpoints must have the header `Authorization: Token XXXXXXXXXXXXXXXXXX`
...
* `/items/{item_name}/assets/{asset_tag}`
  ...
  * PUT: change the value of asset fields with current tag `{asset_tag}` associated with object with an  `{item_name}`  
